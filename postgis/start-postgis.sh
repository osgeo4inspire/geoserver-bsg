...
then
    ...
    #Injecting content from WFS schema
    su - postgres -c "createdb -O $POSTGRES_USER -T template_postgis postgis"
    su - postgres -c "psql postgis -c 'CREATE SCHEMA inspirewfs AUTHORIZATION $POSTGRES_USER;' "
    su - postgres -c "PGPASSWORD=$POSTGRES_PASS  pg_restore -h localhost  --username 'docker'  --dbname  postgis --no-password   --no-owner --no-privileges --no-tablespaces  --schema inspirewfs --verbose /inspirewfs.backup"
    su - postgres -c "psql postgis -c \"CREATE ROLE ows_reader LOGIN PASSWORD 'docker' \" "
    su - postgres -c "psql postgis -c \"GRANT USAGE ON SCHEMA inspirewfs TO ows_reader;\" "
    su - postgres -c "psql postgis -c \"GRANT SELECT ON ALL TABLES IN SCHEMA inspirewfs TO ows_reader;\" "

 #su - postgres -c "psql postgis -c \"CREATE ROLE ows_reader LOGIN PASSWORD 'docker'  VALID UNTIL 'infinity' \" "
